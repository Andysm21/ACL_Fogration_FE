...
const CorporateRequests:React.FC<{courses}> = ({courses}) => {
    ...
    <div className="grid grid-cols-2 text-white  bg-bc gap-4 h-screen">
      {/* {problems.map((problem,index) => ( */}
        ...
        <div className="flex gap-4 flex-row-1 bg-black3 justify-between mx-6 my-4 rounded-lg py-3 px-4 h-20">
            ...
            <div>{/*user?user.CorporateUser_ID*/ "17"}
            </div></div>


          <div className="text-l">User company: 
          <div>{/*user?user.CorporateUser_Corporate*/"Siemens"}
          </div></div>

          <div className="text-l">Requested course: 
          <div>{/*course?course.Course_Title*/"Embedded Systems"}
            </div></div>
