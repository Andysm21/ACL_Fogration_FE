...
const AddInstructor: React.FC<Props> = ({ handleClose, isOpen }) => {
  ...
  const fullScreen = useMediaQuery(theme.breakpoints.down("md"));
    ...
    var status = '';
    const handleSubmit = () => {
      axios.post('http://localhost:8000/createInstructor', {Instructor_username: username, Instructor_Password: password})
      .then((response) => {
        if(response.data == "1"){
  ...
  return (
    <div>
      ...
      >
        <DialogTitle className="bg-bc text-white "id="responsive-dialog-title">{"New Instructor"}</DialogTitle>
